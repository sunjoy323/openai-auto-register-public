...
LOG_PATH = os.path.join(SCRIPT_DIR, "team_login_history.log")
...
def save_token(token_json: str, token_dir: str) -> tuple[str, str, str]:
    ...
    file_name = f"team_token_{email.replace('@', '_')}_{int(time.time())}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="邮箱列表 OTP 登录并换取 Team Token")
    parser.add_argument("--config", default=CONFIG_PATH, help="配置文件路径，默认读取同目录 config.json")
    args = parser.parse_args()

    cfg = load_config(args.config)
    login_cfg = cfg.get("team_login", {}) if isinstance(cfg.get("team_login"), dict) else {}
    ...
    prefer_team = to_bool(login_cfg.get("prefer_team"), True)
    proxy = cfg.get("proxy")
    ...
    if not isinstance(emails, list) or not emails:
        raise SystemExit("[Error] 请在 config.json 的 team_login.emails 中配置邮箱列表")

    emails = [str(item).strip() for item in emails if str(item).strip()]
    if not emails:
        raise SystemExit("[Error] team_login.emails 里没有有效邮箱")

    print("[Info] 邮箱列表 OTP 登录换 Token")
    print(f"[Info] 邮箱数量: {len(emails)}")
    print(f"[Info] 代理: {proxy or '无'}")
    print(f"[Info] Team 优先: {prefer_team}")
