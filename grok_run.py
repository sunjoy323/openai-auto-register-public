...
Grok 自动注册脚本
用法:
  python3 grok_run.py                          # 循环注册
  python3 grok_run.py --once                   # 只跑一次
  python3 grok_run.py --count 5                # 跑5个
  python3 grok_run.py --parallel 3             # 3线程无限循环
"""
import json
import os
import sys
...
# 同目录下的注册脚本
from grok_register import run as register_run

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(SCRIPT_DIR, "config.json")
TOKENS_DIR = os.path.join(SCRIPT_DIR, "grok_tokens")
LOG_PATH = os.path.join(SCRIPT_DIR, "grok_history.log")
...
def load_config():
    if not os.path.exists(CONFIG_PATH):
        return {"proxy": "", "register": {"sleep_min": 5, "sleep_max": 30}}
...
def run_once(cfg: dict, tag: str = "") -> bool:
    ...
    prefix = reg_cfg.get("email_prefix", "gk")
    ...
    with open(fpath, "w", encoding="utf-8") as f:
        f.write(token_json)
    _tprint(f"{tag}[*] Grok Token 已保存: {fpath}")
...
def main():
    parser = argparse.ArgumentParser(description="Grok 自动注册工具")
